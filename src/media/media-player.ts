...
export class MediaPlayer {
                ...
                if (this.channel && item)
                    this.channel.send(
                        createEmbed()
                            ...
                            .addFields({ name: 'Title:', value: item.name }, { name: 'Position:', value: `${this.queue.indexOf(item) + 1}`, inline: true }, { name: 'Requested By:', value: item.requestor, inline: true })
                    );
            })
            .catch((err) => {
    ...
    dispatchStream(stream: Readable, item: MediaItem) {
        ...
        this.dispatcher.on('start', async () => {
            ...
            this.determineStatus();
            if (this.channel) {
                const msg = await this.channel.send(createEmbed().setTitle('▶️ Now playing').setDescription(`${item.name}`).addField('Requested By', `${item.requestor}`));
            }
        });
    ...
    play() {
        if (this.queue.length == 0 && this.channel) this.channel.send(createInfoEmbed(`Queue is empty! Add some songs!`));
